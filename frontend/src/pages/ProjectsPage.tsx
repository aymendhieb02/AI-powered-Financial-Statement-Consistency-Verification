  ...
  return (
    <section className="space-y-6">
      <PageHeader
        eyebrow="Portfolio"
        title="Projects"
        description="Project workspaces group documents, extraction evidence, verification runs, and generated audit reports."
        actions={<><Input className="w-64" value={name} onChange={(e) => setName(e.target.value)} /><Button onClick={add}><Plus size={15} /> New project</Button></>}
      />
      <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
        {visible.map((project, index) => (
          <Link key={project.id} to="/project" onClick={() => localStorage.setItem('finverify_project_id', project.id)}>
            <Card className="group p-5 transition hover:border-slate-300 hover:shadow-md">
              ...
              <div className="mt-5 grid grid-cols-3 gap-3 text-xs">
                <div><div className="text-slate-500">Documents</div><div className="font-semibold text-slate-900">{4 + index}</div></div>
                <div><div className="text-slate-500">Risk</div><div className="font-semibold text-slate-900">{18 + index * 7}</div></div>
                <div><div className="text-slate-500">Confidence</div><div className="font-semibold text-slate-900">{98 - index}%</div></div>
              </div>
              <div className="mt-4"><ProgressBar value={82 - index * 8} /></div>
              <div className="mt-4 flex items-center justify-between"><StatusBadge status="completed" /><span className="text-xs text-slate-500">Last run today</span></div>
            </Card>
          </Link>
        ))}
      </div>
