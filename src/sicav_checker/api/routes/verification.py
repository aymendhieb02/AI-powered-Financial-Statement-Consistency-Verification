...
from sicav_checker.api.schemas.api_models import (
    ComparisonEvidenceModel,
...
def get_evidence_list(project_id: str, run_id: str, service: ProjectService = Depends(get_project_service)) -> EvidenceListResponse:
    items = service.list_evidence(project_id, run_id)
    return EvidenceListResponse(items=[ComparisonEvidenceModel(**item) for item in items], total=len(items))


@router.get("/{run_id}/evidence/{evidence_id}", response_model=ComparisonEvidenceModel)
def get_evidence_detail(project_id: str, run_id: str, evidence_id: str, service: ProjectService = Depends(get_project_service)) -> ComparisonEvidenceModel:
    return ComparisonEvidenceModel(**service.get_evidence(project_id, run_id, evidence_id))
