...
def test_wrong_sign_detected() -> None:
    ...
    assert any(item.canonical_label == "actif_net" and item.status == Status.CARRY_FORWARD_MISMATCH for item in results)


def test_swapped_columns_detected() -> None:
    old, new = corrupted_documents("swapped_columns")
    results = compare_documents(old, new)
    assert any(item.status == Status.CARRY_FORWARD_MISMATCH for item in results)


def test_decimal_issue_detected() -> None:
    old, new = corrupted_documents("decimal_issue")
    results = compare_documents(old, new)
    assert any(item.canonical_label == "total_actif" and item.status == Status.CARRY_FORWARD_MISMATCH for item in results)
